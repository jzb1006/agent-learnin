...
Day 04 已实现基础 REST API，Day 06 已接入 Spring AI ChatClient 的最小业务边界，Day 07 已把客服 Prompt 收敛为可版本化、可测试的行为契约，Day 08 已补 `/chat` 本地意图识别 fallback，Day 09 已统一结构化客服响应，Day 12 已实现只读订单查询工具，Day 13 已实现只读课程目录工具和低风险人工转接工具：
...
| `course_catalog(tenantId, category?)` | 只读工具；返回当前租户启用课程和政策目录 |
| `handoff_to_human(tenantId, conversationId, reason, orderId?)` | 低风险写工具；只创建本地人工转接记录，不触发外部真实派单 |
...
- 不调用 MCP Server。
- 不执行真实退款、取消或改签。
- `handoff_to_human` 只创建本地记录，不创建真实人工客服工单。
