...
Day 04 已实现基础 REST API，Day 06 已接入 Spring AI ChatClient 的最小业务边界，Day 07 已把客服 Prompt 收敛为可版本化、可测试的行为契约，Day 08 已补 `/chat` 本地意图识别 fallback，Day 09 已统一结构化客服响应，Day 15 已把工具调用结果接入 `/chat`，Day 23 已让对话编排层通过 `McpToolClient` 调用真实 stdio MCP service，Day 24 已补短期会话记忆与上下文压缩：

| 接口 | 当前行为 |
| --- | --- |
| `GET /health` | 返回 `status=UP` 和 `service=customer-agent-app` |
| `GET /api/orders/{orderId}` | 返回内存 mock 订单；不存在时返回 `ORDER_NOT_FOUND` |
| `POST /chat` | 先经 `IntentRouter` 分流到 `KNOWLEDGE_QA`、`ORDER_LOOKUP`、`REFUND_OR_CANCEL`、`HUMAN_HANDOFF` 或 `DIRECT`；同租户同会话可复用最近订单上下文；通过 `McpToolClient` 调用 `kb_search`、`order_lookup`、`refund_policy_check`；统一返回 `route / answer / sources / riskLevel / nextActions / traceId / toolCalls / conversationId / memorySummary` |
